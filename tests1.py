...
def test_null_values(connect):
    ...
    cursor.execute(
        "SELECT count(*) FROM AdventureWorks2012.Person.Address WHERE AddressLine1 IS NULL OR TRIM("
        "AddressLine1) = ''"
    )
...
def test_duplicates_check(connect):
    cursor = connect.cursor()
    cursor.execute(
        "SELECT count(*) FROM (SELECT UPPER(TRIM([UnitMeasureCode])) AS CLEAN_UMC, COUNT(*) AS CNT FROM ["
...
def test_date_less_now(connect):
    cursor = connect.cursor()
    cursor.execute(
        "SELECT COUNT(*) FROM [AdventureWorks2012].[Production].[UnitMeasure] WHERE [ModifiedDate] > "
        "CURRENT_TIMESTAMP;"
    )
    row_count = cursor.fetchone()[0]
